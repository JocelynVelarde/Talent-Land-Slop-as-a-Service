...
Datos sintéticos generados con `tecovolt_synth.py`, calibrados con **PicoScope 2208B** usando señales reales de la red eléctrica de Puebla, MX. El generador de señales arbitrarias (AWG) del PicoScope permite crear condiciones controladas de cada clase.

### Resultados de entrenamiento

- **1er entrenamiento:** Accuracy float32 = 92.6%. Confusión principal: `sag_leve` ↔ `normal` en bordes de RMS.
- **2do entrenamiento:** Accuracy float32 = **94.5%**. Mejoras: +100 ciclos, Flatten layer, dataset ampliado en clases con traslape.

{{% alert title="Pendiente" color="warning" %}}
Reentrenamiento con datos reales capturados del ACS712 y ZMPT101B en el Arduino Uno Q.
{{% /alert %}}
...
- `avg_temperature` por ventana de 10 muestras
- `avg_humidity` por ventana de 10 muestras
...
Datos reales capturados con PicoScope 2208B:
- **Baja:** rawRMS ≈ 0.0 (sin carga)
- **Alta:** rawRMS ≈ 169.5 (dos calentadores ~1900 W)
- **Media:** estimado pendiente de captura real

**1er entrenamiento:** Accuracy = 87.5%. Confusión: `media` vs. `alta` (clase media usa rawRMS estimado, no real).

{{% alert title="En curso" color="info" %}}
Captura real de clase 'media' con ACS712 y carga de ~950 W para mejorar el modelo.
{{% /alert %}}
...
### `tecovolt_block` (Voltaje)

Extrae las 5 features físicas relevantes para clasificación de anomalías de voltaje. Implementado en Python, empaquetado en Docker y registrado en Edge Impulse Studio como bloque personalizado.

### `tecotemp_block` (Temperatura)

Procesa las ventanas de temperatura y humedad del BMP280 con un Flatten block optimizado para señales de baja frecuencia.
